...
async def create_ticket(user_id: int, problem_type: str, description: str) -> Optional[int]:
        ...
        async with db_pool.acquire() as conn:
            ...
            ticket = await conn.fetchrow(
                """INSERT INTO tickets (user_id, ticket_number, problem_type) 
                   VALUES ($1, $2, $3) RETURNING id, ticket_number""",
...
async def select_problem_type(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Error in select problem type: {e}")
        await callback.answer("Произошла ошибка")
...
async def process_ticket_description(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not problem_type:
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> '
                f'Ошибка: тип проблемы не выбран. Начните создание тикета заново.',
        ...
        if not ticket_id:
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> '
                f'Ошибка при создании тикета. Попробуйте позже.',
                reply_markup=main_menu_keyboard(user_data['is_admin'])
            )
            await state.clear()
            return
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> '
            f'Произошла ошибка при создании тикета. Попробуйте позже.',
            reply_markup=main_menu_keyboard(False)
        )
        await state.clear()
...
# Остальные обработчики (статистика, рассылка, управление поддержкой, имена)
...
async def broadcast_send(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Error in broadcast send: {e}")
...
async def admin_manage_support(callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in manage support: {e}")
        await callback.answer("Произошла ошибка")
...
async def add_support_user(message: Message):
    ...
    try:
        ...
        logger.error(f"Error adding support user: {e}")
...
async def remove_support_user(message: Message):
    ...
    try:
        ...
        logger.error(f"Error removing support user: {e}")
        await message.answer("Произошла ошибка.")
...
async def admin_display_names(callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in display names: {e}")
        await callback.answer("Произошла ошибка")
...
async def back_to_admin(callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in back to admin: {e}")
        await callback.answer("Произошла ошибка")
...
async def set_display_name_start(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Error in set display name start: {e}")
        await callback.answer("Произошла ошибка")
...
async def process_display_name(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Error processing display name: {e}")
        await message.answer("Произошла ошибка.")
        await state.clear()
...
async def back_to_support(callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in back to support: {e}")
        await callback.answer("Произошла ошибка")
...
async def back_to_tickets(callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in back to tickets: {e}")
        await callback.answer("Произошла ошибка")
